...
const len = (s) =>
	typeof s === "string" ? s.length : "size" in s ? s.size : s.length;
...
const _da =
	CC_DEPENDENT_VOWEL | CF_POS_ABOVE | CF_DOTTED_CIRCLE | CF_ABOVE_VOWEL;
	...
	while (cursor < charCount) {
		...
		while (cursor < charCount) {
			...
			if (kChar === _xx) reserved = curChar;
				...
				// Khmer split vowel, see _da
				vowelBefore = SRAE;
				vowelAbove = sraEcombining[curChar];
			} else if (kChar === _vr) {
				// Khmer split vowel, see _dr
				vowelBefore = SRAE;
